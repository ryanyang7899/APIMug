...
build/APIMug.app/Contents/MacOS/APIMug --lbqhtest [baseURL] [apiKey]    # 联并千行余额刷新链路自测
...
- **联并千行（lbqh）是独立开关**：不是真实站点，勾选时由 MonitorController.refreshLBQH() 用固定虚拟 ID（`MonitorController.lbqhSiteID`）查询并存 snapshot，复用站点的展示/低余额提醒/折线图/用量追踪。配置在 `AppConfig.lbqh`（`LBQHConfig`），走 `GET {base}/api/balance` + `X-API-Key` 头（`APIService.fetchLBQH`，注意 lbqh 不走 Bearer）。
