...
设置 → 通用设置 → 「监控联并千行余额」勾选后，按配置的服务地址/API 令牌查询联并千行 MaaS 平台余额（**仅余额**，元/CNY）：

- 数据来源：`GET {base}/user/balance`（多用户监控服务，默认 `http://localhost:8100`），需 `Authorization: Bearer <令牌>`（令牌在服务网页端「API 令牌」页创建，格式 `lbqh-<32位hex>`）
- 定时刷新走 `GET /user/balance` 读缓存（无成本）；菜单栏「立即刷新」额外触发 `POST /api/balance/fetch` 同步抓一次（约 10~20 秒，消耗验证码识别成本）
- 返回结构与 DeepSeek `/user/balance` 完全一致，仅取余额
- 余额行与普通站点一样展示在菜单弹窗（`✓ 联并千行 — 余额 ¥999.35`），聚合标题带首字符「联」（`联 ¥999.35`）
- 参与低余额提醒与近 7 日用量折线图（与 DeepSeek 等余额型平台一致，按刷新周期查询）

功能：多站点监测、菜单栏聚合显示、**本日/本月用量统计（余额基准法）**、低余额通知（按天去重）、定时自动刷新（默认 30 分钟）、设置窗口、**检查更新**、配置持久化（UserDefaults）。

## 检查更新

- **开机自动启动**：设置 → 通用设置 → 「开机自动启动」勾选即生效（无需去系统设置操作）
...
菜单栏显示内容由**每个站点自己的**勾选控制（设置 → 监测站点 → 该站点 → 菜单栏显示：余额 / 本日用量 / 本月用量），每站点独立。

被选中展示的站点，其数据会**带站点名首字符**标注在菜单栏（如 `D ¥40.74` 表示 DeepSeek 的余额、`K 今日 ¥3.89` 表示 Kimi 的本日用量），一眼区分是哪个站点的数据。多站点可同时勾选、多项并列显示。全部不勾时菜单栏显示 ☕。站点详情行始终显示 余额 + 本日 + 本月。
...
菜单弹窗顶部显示**近 7 日用量折线图**（每日用量历史自动持久化，随使用天数积累）。
